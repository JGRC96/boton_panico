...
+function ($) {
  ...
  + function ($) {
    ...
    var Selector = {
      ...
      expanded: '',
      layoutFixed: '.fixed',
      toggleButtonSm: '.toggle-btn-sm' // Nuevo selector para el botón pequeño
    }
    ...
    // Data API 
    $(document).on('click', Selector.button + ', ' + Selector.toggleButtonSm, function (e) {
      e.preventDefault();

      // Alternar el menú
      Plugin.call($(this), 'toggle');

      // Actualizar el ícono del botón al alternar el menú
      var $icon = $(this).find('span'); // Encuentra el ícono dentro del botón

      if ($(window).width() > Default.collapseScreenSize) {
        // Pantallas grandes (sidebar-collapse)
        ...
        // Pantallas pequeñas (sidebar-open)
        if ($('body').hasClass(ClassName.open)) {
          $icon.removeClass('fa-solid fa-align-justify').addClass('fa-solid fa-outdent');
        } else {
          $icon.removeClass('fa-solid fa-outdent').addClass('fa-solid fa-align-justify');
        }
      }
    });
    

    // Asegurar que el ícono esté en el estado correcto al cargar la página
    $(window).on('load', function () {
      const buttons = [$(Selector.button), $(Selector.toggleButtonSm)];
      buttons.forEach(($button) => {
        const $icon = $button.find('span'); // Encuentra el ícono dentro del botón
